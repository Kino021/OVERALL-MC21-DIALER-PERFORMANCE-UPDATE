    ...
    if df.empty:
        ...
        def calculate_combined_summary(df):
            ...
            for date, group in df.groupby(df['Date'].dt.date):
                ...
                total_balance = group[(group['Status'].str.contains('PTP', na=False)) & (group['PTP Amount'] != 0)]['Balance'].sum()
        ...
        def calculate_predictive_summary(df):
            ...
            for date, group in df_filtered.groupby(df['Date'].dt.date):
        ...
        st.write(predictive_summary_table, container_width=True)
        ...
        def calculate_manual_summary(df):
            ...
            df_filtered = df[df['Remark Type'].isin(['Manual'])]

            for date, group in df_filtered.groupby(df['Date'].dt.date):
                accounts = group[group['Remark Type'].isin(['Manual'])]['Account No.'].nunique()
                total_dialed = group[group['Remark Type'].isin(['Manual'])]['Account No.'].count()
                connected = group[group['Call Status'] == 'CONNECTED']['Account No.'].nunique()
                penetration_rate = (total_dialed / accounts * 100) if accounts != 0 else None
                connected_acc = group[group['Call Status'] == 'CONNECTED']['Account No.'].count()
                connected_rate = (connected_acc / total_dialed * 100) if total_dialed != 0 else None
        ...
        st.write(manual_summary_table, container_width=True)
        
        # Predictive Per Cycle Summary
        def calculate_predictive_per_cycle_summary(df):
            summary_table = pd.DataFrame(columns=[ 
            ...
            for cycle, group in df_filtered.groupby('Service No.'):
                for date, date_group in group.groupby(group['Date'].dt.date):  
                    ...
                    ptp_amount = date_group[(date_group['Status'].str.contains('PTP', na=False)) & (date_group['PTP Amount'] != 0)]['PTP Amount'].sum()
                    ptp_rate = (ptp_acc / connected * 100) if connected != 0 else None
                    ...
                    call_drop_ratio = (system_drop / connected_acc * 100) if connected_acc != 0 else None
                    total_balance = date_group[(date_group['Status'].str.contains('PTP', na=False)) & (date_group['PTP Amount'] != 0)]['Balance'].sum()
        ...
        predictive_per_cycle_summary = calculate_predictive_per_cycle_summary(df)
        st.write(predictive_per_cycle_summary, container_width=True)
