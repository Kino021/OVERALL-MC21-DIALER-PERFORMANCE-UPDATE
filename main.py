...
if uploaded_file is not None:
    ...
    if df.empty:
        ...
        # Function to calculate the summary tables with collectors count
        def calculate_summary_with_collectors(df, remark_type=None):
            ...
            for date, group in df.groupby(df['Date'].dt.date):
                # Filter the group by remark type (if provided)
                if remark_type:
                    group = group[group['Remark Type'] == remark_type]
        ...
        overall_combined_summary = calculate_summary_with_collectors(df)
        overall_predictive_summary = calculate_summary_with_collectors(df, remark_type="Predictive")
        overall_manual_summary = calculate_summary_with_collectors(df, remark_type="Manual")
