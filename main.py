...
def calculate_combined_summary(df):
    summary_table = pd.DataFrame(columns=[ 
        'Day', 'CLIENT', 'CAMPAIGN', 'ACCOUNTS', 'TOTAL DIALED', 'PENETRATION RATE (%)', 'CONNECTED #', 
    ...
    for date, group in df.groupby(df['Date'].dt.date):
        # Debugging step: Check columns in each group
        print(group.columns)  # Print to see the columns for the current group
        
        # Ensure 'CLIENT' exists and handle missing columns
        client = group.get('CLIENT', 'Unknown Campaign')
        ...
        summary_table = pd.concat([summary_table, pd.DataFrame([{
            'Day': date,
            'CLIENT': client,  # Now safely adding CLIENT
