...
if uploaded_file is not None:
    ...
    def calculate_combined_summary(df):
        ...
        for date, group in df.groupby(df['Date'].dt.date):
            ...
            summary_table = pd.concat([summary_table, pd.DataFrame([{
                ...
                'PENETRATION RATE (%)': f"{round(penetration_rate)}%" if penetration_rate is not None else '0%',
                'CONNECTED #': connected,
                'CONNECTED RATE (%)': f"{round(connected_rate)}%" if connected_rate is not None else '0%',
                'CONNECTED ACC': connected_acc,
                'PTP ACC': ptp_acc,
                'PTP RATE': f"{round(ptp_rate)}%" if ptp_rate is not None else '0%',
                'CALL DROP #': call_drop_count,
                'CALL DROP RATIO #': f"{round(call_drop_ratio)}%" if call_drop_ratio is not None else '0%',
            }])], ignore_index=True)
    ...
    # Add code for other tables as needed...
