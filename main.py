...
def load_data(uploaded_file):
    ...
    df = df[~df['Remark By'].isin(['FGPANGANIBAN', 'KPILUSTRISIMO', 'BLRUIZ', 'MMMEJIA', 'SAHERNANDEZ', 'GPRAMOS'
                                   ...
                                   , 'DASANTOS', 'SEMIJARES', 'GMCARIAN', 'RRRECTO', 'JMBORROMEO', 'EUGALERA','JATERRADO','LMLABRADOR'])]
    return df
...
if uploaded_file is not None:
    ...
    def calculate_combined_summary(df):
        ...
        for date, group in df.groupby(df['Date'].dt.date):
            ...
            call_drop_count = group[group['Call Status'] == 'NEGATIVE CALLOUTS']['Account No.'].count()
    ...
    def calculate_summary(df, remark_type, remark_by=None):
        ...
        for date, group in df.groupby(df['Date'].dt.date):
            ...
            call_drop_count = group[(group['Remark Type'] == remark_type) | ((group['Status'] != 'NEGATIVE CALLOUTS'))]['Account No.'].count()
            call_drop_ratio = (call_drop_count / connected * 100) if connected != 0 else None
